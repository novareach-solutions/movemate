  ...
  async ({ orderId, url }, { rejectWithValue }) => {
    try {
      ...
      if (response.data.success) {
        ...
        return rejectWithValue(response.data.message || 'Failed to update photo.');
      }
    } catch (error: any) {
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchOngoingOrder.fulfilled, (state, action: PayloadAction<SendPackageOrder | null>) => {
        state.loadingOngoingOrder = false;
        state.ongoingOrder = action.payload;
      })
      .addCase(fetchOngoingOrder.rejected, (state, action) => {
        state.loadingOngoingOrder = false;
        state.errorOngoingOrder = action.payload || 'Failed to fetch ongoing order.';
      })
      ...
      .addCase(updateItemVerifiedPhoto.rejected, (state, action) => {
        state.updatingItemVerifiedPhoto = false;
        state.errorUpdatingItemVerifiedPhoto = action.payload || 'Failed to update item verified photo.';
      })
      ...
      .addCase(updateOrderStatus.fulfilled, (state, action: PayloadAction<SendPackageOrder>) => {
        ...
        if (action.payload.status === OrderStatusEnum.PICKEDUP_ORDER) {
          ...
          state.deliveryDetails = {
            dropoffAddress: action.payload.dropLocation?.addressLine1 || 'N/A',
