...
import { dbRepo, dbReadRepo } from "../../database/database.service";
...
export class AgentVehicleService {
  ...
  async createVehicle(agentId: number, vehicleData: TAgentVehicle): Promise<AgentVehicle> {
    const existingVehicle = await dbReadRepo(AgentVehicle).findOne({
      where: { licensePlateNumber: vehicleData.licensePlateNumber },
    });
    if (existingVehicle) {
      throw new UserLicensePlateAlreadyExistsError("This Car is already registered");
    }
    const newVehicle = dbRepo(AgentVehicle).create({ ...vehicleData, agentId });
    return await dbRepo(AgentVehicle).save(newVehicle);
  }

  async updateVehicle(agentId: number, vehicleId: number, updateData: Partial<TAgentVehicle>): Promise<UpdateResult> {
    return await dbRepo(AgentVehicle).update({ id: vehicleId, agentId }, updateData);
  }
  ...
  async deleteVehicle(agentId: number, vehicleId: number): Promise<DeleteResult> {
  ...
  async updateVehicleApprovalStatus(
    ...
    const vehicle = await dbReadRepo(AgentVehicle).findOne({ where: { id: vehicleId, agentId } });
    if (!vehicle) {
